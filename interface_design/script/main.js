/*
This code is based on the three-mesh-bvh example: https://github.com/gkjohnson/three-mesh-bvh/blob/master/example/selection.js
(original author: Garrett Johnson)

The functionality is modified to fit the purpose of this project by Takanori Fujiwara.
*/
...
const setOfSelectedGeometryIndices = [];
let newIndexAttr;
...
function init(geometry) {
	...
	mesh.geometry.rotateX(-1.5708); // rotate 90 degree
	// get bonding box
	mesh.geometry.computeBoundingBox();
	const lengths = mesh.geometry.boundingBox.max.sub(mesh.geometry.boundingBox.min);
	const maxLength = Math.max(...Object.values(lengths));
	mesh.geometry.translate(-mesh.geometry.boundingBox.min.x - lengths.x / 2, -mesh.geometry.boundingBox.min.y - lengths.y / 2, -mesh.geometry.boundingBox.min.z - lengths.z / 2);
	mesh.geometry.scale(5 / maxLength, 5 / maxLength, 5 / maxLength);
	...
	newIndexAttr = highlightMesh.geometry.index;
	...
	const downloadButton = {
		add: function () {
			...
			// navigator.clipboard.writeText(JSON.stringify(setOfSelectedGeometryIndices));

			const file = new File([JSON.stringify(setOfSelectedGeometryIndices)], 'selection.json', {
				type: 'text/plain',
			})
	...
	gui.add(downloadButton, 'add').name('download selected vertex indices');
	...
	renderer.domElement.addEventListener('dblclick', e => {
		highlightMesh.geometry.drawRange.count = 0;
		setOfSelectedGeometryIndices.length = 0;
...
function updateSelection() {
	...
	if (indices.length && params.selectModel) {
		...
		const selectedGeometryIndices = [];
		// update the highlight mesh
		for (let i = 0, l = indices.length; i < l; i++) {
			const i2 = indexAttr.getX(indices[i]);
			selectedGeometryIndices.push(i2);
			newIndexAttr.setX(i + highlightMesh.geometry.drawRange.count, i2);
		}
		// highlightMesh.geometry.drawRange.count = indices.length;
		highlightMesh.geometry.drawRange.count += indices.length;
		newIndexAttr.needsUpdate = true;

		if (selectedGeometryIndices.length > 0) {
			setOfSelectedGeometryIndices.push(selectedGeometryIndices);
		}
	}
}
