...
if __name__ == "__main__":
    ...
    # subdivided surface has open edges so read non-subdivided one
    surface = pv.get_reader(f"../models/{mesh_file_name}.stl").read()
    surface = surface.scale([1, 1, 1.5])
    surface.save(f"../models/{mesh_file_name}_scaled.stl")
    ...
    ## 2. Link path finding
    # increase resolution for z
    voxels = gu.surface_to_voxels(surface, resolution=np.array([150, 150, 400]))

    # from sw_sensing.geometry import prepare_node_objects
    ...
    # )
    # p = pv.Plotter()
    # p.add_mesh(surface, show_edges=False, opacity=0.9, color="green", lighting=True)
    # p.add_mesh(combine_polydata_objects(node_objects), color="red")
    # p.show()
    ...
    set_of_link_path_positions = graph_based_path_finding(
        ...
        g=voxel_graph,
        min_space=22,
        min_space_violation_penalty=500,
    )
    ...
    (
        ...
        link_kwargs={
            **default_args(generate_all_geometries)["link_kwargs"],
            "radius": [1.5] + [3.5] * (len(set_of_link_path_positions) - 1),
        },
        resistor_trace_fill_kwargs={
            **default_args(generate_all_geometries)["resistor_trace_fill_kwargs"],
            "aiming_resistance": 900e3,
            "learning_rate": 0.2,
            "max_iterations": 100,
            # "sep_dist_thres": 10.0,
            "no_overlap_margin": 5.0,
            "max_fill_area_radius": [1.0]
            + [3.0] * (len(set_of_link_path_positions) - 1),
            "min_dy": 1.2,
            "min_dz": 1.2,
        },
    ...
    #     trace_resistance_candidates=[900e3 * 0.23],
    # )
    # print(
