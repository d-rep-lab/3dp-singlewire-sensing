...
if __name__ == "__main__":
    ...
    surface = pv.get_reader(f"../models/{mesh_file_name}_scaled.stl").read()
    surface = surface.scale(1.5)
    ...
    node_order = node_order_selection(
        initial_order=[0, 4, 2, 5, 6, 1, 3],  # [0, 2, 3, 5, 6, 4, 1],
        method="asis",
    ...
    (
        ...
        link_kwargs={
            **default_args(generate_all_geometries)["link_kwargs"],
            "radius": [1.5] + [4.5] * (len(set_of_link_path_positions) - 1),
        },
        resistor_trace_fill_kwargs={
            **default_args(generate_all_geometries)["resistor_trace_fill_kwargs"],
            "aiming_resistance": 530e3,
            "learning_rate": 0.1,
            "max_iterations": 300,
            "sep_dist_thres": 50.0,
            "no_overlap_margin": 4.2,
            "max_fill_area_radius": [1.0]
            + [4.0] * (len(set_of_link_path_positions) - 1),
            "min_dy": 1.2,
            "min_dz": 1.2,
        },
    ...
    #     trace_resistance_candidates=[530e3 * 0.23],
    # )
    # print(
